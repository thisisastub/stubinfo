...
const IX_TYPES = [
  ...
  { id: "creative", label: "Creative", desc: "Design and visual assets", accent: "#A8873C", steps: [
    { title: "The asset", fields: [O("kind", "Asset type", ["Static image", "Carousel", "Animation", "Print"], { req: 1 }), M("sizes", "Sizes needed", ["1080\xD71080 feed", "1080\xD71920 story", "1200\xD7628 link card", "1600\xD7900 web hero", "8.5\xD711 print"], { req: 1, help: "Pick every size. We build to spec, not to guess." })] },
  ...
  { id: "social", label: "Social", desc: "Social posts and graphics", accent: "#1E8A62", steps: [
    ...
    { title: "Assets", fields: [O("needsGraphic", "The graphic", ["Design builds it", "I have the file"], { req: 1 }), M("sizes", "Sizes", ["1080\xD71080 feed", "1080\xD71920 story", "1200\xD7628 link card"], { when: (a) => a.needsGraphic === "Design builds it", req: 1 }), D("goLive", "Go-live date", { req: 1 })] }
  ] },
...
const IX_DIMS = {
  "1080\xD71080 feed": [1080, 1080, "px", "Feed"],
  "1080\xD71920 story": [1080, 1920, "px", "Story"],
  "1200\xD7628 link card": [1200, 628, "px", "Link card"],
  "1600\xD7900 web hero": [1600, 900, "px", "Web hero"],
  "8.5\xD711 print": [8.5, 11, "in", "Print"]
};
...
const DEMO = {
  social: {
    ...
    format: "Still graphic",
    copyInGraphic: DEMO_LINE,
    ...
    ctaUrl: "dollarous.example/close-faster/calculator",
    needsGraphic: "Design builds it",
    sizes: ["1080\xD71080 feed", "1080\xD71920 story"],
    goLive: "2026-09-29"
  },
  creative: {
    kind: "Static image",
    sizes: ["1080\xD71080 feed", "1080\xD71920 story", "1200\xD7628 link card"],
    copyInGraphic: DEMO_LINE,
    cta: "Price your own close",
    approver: "Marisol Ferreira"
  },
  email: {
    campaign: "Close Faster",
    segment: "Mid-market controllers, 200\u2013800 invoices a month",
    goLive: "2026-10-01",
    templated: "Use a template",
    template: "Single-CTA plain",
    subject: "Your close is not slow. Your exceptions are.",
...
const filled = (f, ans) => {
  const v2 = ans[f.id];
  return f.type === "many" ? Array.isArray(v2) && v2.length > 0 : !!(v2 && String(v2).trim());
};
  ...
  if (f.type === "one" || f.type === "many") {
    ...
    ctl = React.createElement("div", { className: "ix-opts" }, f.opts.map((o) => {
      ...
      return React.createElement("button", {
        key: o,
        type: "button",
        className: "ix-opt" + (on ? " on" : "") + (tap === o ? " tap" : ""),
        "aria-pressed": on,
        onClick: () => onChange(f.id, f.type === "many" ? on ? arr.filter((x) => x !== o) : arr.concat([o]) : o)
      }, o);
    }));
  } else if (f.type === "area") {
...
function TypePicker({ sel, setSel, onBegin, onDemo, demo, tap, press }) {
  ...
  return React.createElement(
    ...
    React.createElement("div", { className: "ix-tiles" }, IX_TYPES.map((t) => {
      ...
      return React.createElement(
        "button",
        {
          key: t.id,
          type: "button",
          className: "ix-tile" + (on ? " on" : "") + (tap && tap.tile === t.id ? " tap" : ""),
          style: { "--tc": t.accent },
          "aria-pressed": on,
          disabled: !!demo,
    ...
    tip ? React.createElement(
      ...
      React.createElement("span", null, React.createElement("b", null, "Add 3\u20135 content needs."), " Campaigns that ship on time usually move a single message across a few formats at once.")
    ) : null,
    React.createElement("div", { className: "ix-chips" }, sel.length === 0 ? React.createElement("span", { className: "ix-chips-empty" }, "Nothing selected yet.") : sel.map((id) => {
      const t = byId(id);
      return React.createElement(
        "span",
        { key: id, className: "ix-chip", style: { "--tc": t.accent } },
        t.label,
        React.createElement("button", { type: "button", "aria-label": "Remove " + t.label, onClick: () => setSel(sel.filter((x) => x !== id)) }, "\xD7")
      );
    })),
    React.createElement(
      "div",
...
function StepScreen({ t, tIdx, total, step, sIdx, sTotal, ans, setAns, onBack, onNext, onChangeTypes, nextLabel, demo, flash, tap, press }) {
  ...
  const set = (id, v2) => setAns(Object.assign({}, ans, { [id]: v2 }));
  return React.createElement(
    "div",
    { className: "ix-card", style: { "--ix-accent": t.accent } },
    React.createElement(
      "div",
      { className: "ix-top" },
      ...
      React.createElement("span", { className: "ix-step-n" }, "Step ", sIdx + 1, " of ", sTotal, step.optional ? " \xB7 optional" : ""),
      demo ? null : React.createElement("button", { type: "button", className: "ix-change", onClick: onChangeTypes }, "Change types")
    ),
    React.createElement("div", { className: "ix-prog" }, Array.from({ length: sTotal }, (_, i) => React.createElement("i", { key: i, className: i < sIdx ? "done" : i === sIdx ? "now" : "" }))),
...
const Row = ({ k, val, mono }) => React.createElement(
  "div",
  { className: "rq-row" },
  React.createElement("span", { className: "k" }, k),
  React.createElement("span", { className: "v" + (mono ? " mono" : "") }, val)
);
...
function Sheet({ t, ans, all, idx }) {
  ...
  if (t.id === "social") {
    ...
    extra = React.createElement(
      "div",
      { className: "rq-row" },
      React.createElement("span", { className: "k" }, ans.templated === "Use a template" ? "Template \xB7 " + v(ans.template) : "Custom build"),
      React.createElement(
        "div",
        { className: "rq-mail" },
        React.createElement("div", { className: "mh" }, React.createElement("span", null, "DOLLAROUS")),
        React.createElement("div", { className: "hero" }, React.createElement("em", null, ans.needsGraphic === "Yes" ? "1200\xD7628 from Design" : "no hero image")),
        React.createElement(
          "div",
          { className: "mb" },
          ...
          React.createElement("i", null),
          React.createElement("i", { className: "s" }),
    ...
    if (ans.needsGraphic === "Yes") dep = { k: "Dependencies", v: "Design delivers a 1200 \xD7 628 px header. Raised automatically when you answered yes." };
  } else {
    liveSteps(t, ans).forEach((s) => liveFields(s, ans).forEach((f) => rows.push([f.label, v(ans[f.id])])));
  }
  return React.createElement(
    "div",
    { className: "rq", style: { "--rc": t.accent } },
    React.createElement(
      "div",
      { className: "rq-head" },
    ...
    React.createElement(
      "div",
      { className: "rq-body" },
      rows.slice(0, 7).map(([k, val]) => React.createElement(Row, { key: k, k, val })),
      extra
    ),
    dep ? React.createElement(
      "div",
      { className: "rq-dep" },
      React.createElement("span", { className: "k" }, React.createElement(WarnIcon, null), dep.k),
      React.createElement("div", { className: "v" }, dep.v)
    ) : null,
    React.createElement(
      "div",
      { className: "rq-foot" },
      React.createElement("span", null, rows.length + " fields \xB7 1 page"),
...
function IntakeApp() {
    ...
    for (let ti = 0; ti < DEMO_SEL.length; ti++) {
      ...
      for (let si = 0; si < 12; si++) {
        ...
        for (let pass = 0; pass < 8; pass++) {
          const fs = liveFields(steps2[si], acc).filter((f) => !seen.has(f.id) && data[f.id] !== void 0);
          if (!fs.length) break;
          for (const f of fs) {
            if (stop.current) return;
            seen.add(f.id);
            ...
            if (f.type === "one" || f.type === "many") {
              ...
              for (const val of vals) {
                ...
                await wait(300);
                if (f.type === "many") {
                  held = held.concat([val]);
                  acc = Object.assign({}, acc, { [f.id]: held.slice() });
  ...
  const retreat = () => {
    ...
    if (pos[0] > 0) {
      const p = byId(sel[pos[0] - 1]);
      return setPos([pos[0] - 1, liveSteps(p, answers[p.id] || {}).length - 1]);
    }
    setPhase("pick");
  };
  let body;
  if (phase === "start") {
    ...
    body = React.createElement(TypePicker, { key: "pick", sel, setSel, onDemo: runDemo, onBegin: () => {
      setPos([0, 0]);
      setPhase("run");
    }, demo, tap, press });
    ...
    body = React.createElement(Results, { sel, answers, onRestart: () => {
      setAnswers({});
      setPos([0, 0]);
      setPhase("start");
    }, onEdit: () => {
  ...
  return React.createElement(
    ...
    demo ? React.createElement(
      "div",
      { className: "ix-demobar", role: "status" },
      ...
      React.createElement("button", { type: "button", className: "sk", onClick: finishDemo }, "Skip to the end \u2192")
    ) : null,
